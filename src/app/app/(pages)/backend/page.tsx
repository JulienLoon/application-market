...
interface App {
    ...
    image_url: string;
    created_at: string;
    created_by: number; // Ensure this is included
    created_by_username: string; // Ensure this is included
}
                    ...
                    <ul>
                        {latestApps.map((app) => (
                            <li key={app.id} className="border-b py-2">
                                ...
                                <p className="text-gray-600 dark:text-gray-400">Created by: {app.created_by_username} ({app.created_by}) on {new Date(app.created_at).toLocaleDateString()}</p> {/* Updated line */}
                            </li>
                        ))}
                    </ul>
                </div>
